function stm_jacobian = STM_J2_J3(mu,J2,J3,R,r_vec)
%{
...
Inputs:
    ...
    >r:3D cartesian S/C position relative to central body
Outputs:
    >stm_jacobian: Jacobian of partials of acceleration wrt S/C position
                      and mu, J2, J3 (9x9)
%}
...
% Initialize
stm_jacobian = zeros(9);

% Fill in dv/dv portion
stm_jacobian(1:3,4:6) = eye(3);

% Fill in grav coefficients section
stm_jacobian(4:6,7) = -1/r^3 .* [x, y, z]' + ...
...
stm_jacobian(4:6,8) = (3*mu*R^2)/(2*r^5) .* [x*(5*z^2/r^2-1), y*(5*z^2/r^2-1), z*(5*z^2/r^2-3)]'; % J2

stm_jacobian(4:6,9) = (mu*R^3)/(2*r^5) .* [5*x/r*(7*z^3/r^3-3*z/r), 5*y/r*(7*z^3/r^3-3*z/r), 35*z^4/r^4-30*z^2/r^2+3]'; % J3

% Accelerations

da_mudr = [-(r^2-3*x^2), 3*x*y, 3*x*z;
...
stm_jacobian(4:6,1:3) = da_mudr + da_J2dr + da_J3dr;
