function [cov_BPlane,BdotRhat,BdotThat] = BPlaneAnalysis(DCO_t,N,DCO_state,DCO_covariance,constants)
...
[tint, Xint, te, Xe, ie] = ode45(@(t,x) odeSTM_SunEarth3BP_SRP(t,x,constants), tspan, [X0(:);reshape(eye(n),[],1)], options);
...
tspan = [t0 tf+LTOF];
[~, Xint] = ode45(@(t,x) odeSTM_SunEarth3BP_SRP(t,x,constants), tspan, [X0(:);reshape(eye(n),[],1)], options);
