function plot_filter_diagnosticsV3(t,Xhat,P,y,yhat,R,station_id,filter_type,data_type)
%{
Inputs:
    >t: Nx1 time vector [s]
...
parameter_labels = {'C_R','\mu_E','\mu_S'};
...
station_labels = {'x [km]','y [km]','z [km]'};
station_ids = [34 65 13];
...
for j = 1:3
    ...
    idx_start = 9 + (j-1)*3;   % 10, 13, 16
    
    for k = 1:3
        subplot(3,1,k); hold on
        
        idx = idx_start + k;   % actual state index
        
        line = plot(t/3600, +sig3(:,idx), lineStyles{1});
        plot(t/3600, -sig3(:,idx), lineStyles{1}, 'Color', line.Color)
        
        xlabel('Time [hr]')
        ylabel(station_labels{k})
    end
    
    sgtitle(filter_type + " Station " + station_ids(j) + " Position \pm3\sigma Bounds")
