...
while err > tol
    ...
    [~,Xint] = ode45(@(t,x) odeSTM_SunEarth3BP_SRP(t,x,constants),t,[Xbari';reshape(Phii,[],1)],options);
    Xbar = Xint(:,1:n);
    ...
    for i = 1:length(t)
        ...
        % Prefits and update
        Htilde = linearizedH(Xbari(i,:),gs_state(:,i));
        if data_type == "range"
            R_1 = R(1,1);
            Htilde = Htilde(1,:);
            y(:,i,k) = M(3)' - G(2)';
    ...
    % Get covariance, post fits, and corrected state
    for i = 1:length(t)
