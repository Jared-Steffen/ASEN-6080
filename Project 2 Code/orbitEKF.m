...
if LKFinit > 0
    ...
    [~,Xint] = ode45(@(t,x) odeSTM_SunEarth3BP_SRP(t,x,constants),tLKF,Xref,options);
    Xbar = Xint(:,1:n);
...
for i = LKFinit+1:length(t)
    ...
    if i == 1
        ...
        [~,Xint] = ode45(@(t,x) odeSTM_SunEarth3BP_SRP(t,x,constants),current_tstep,Xref,options);
        Xint_end = Xint(end,:);
        Xbar(i,:) = Xint_end(1:n);
    ...
    y(:,i) = M(:,3:4)' - G(:,2:3)';
    Htilde = linearizedH(Xbar(i,:),gs_state);
    Ki = Pbari*Htilde'/(Htilde*Pbari*Htilde' + R);
