function [Htilde] = linearizedH3(t,sc_state,gs_meas_state,constants,station_ids)
%{
Inputs:
    ...
    if current_id == station_ids(i)
        ...
        prhopdotRs_eci = [(-dvx-constants.wE*dy)/rho+(dx*N)/rho^3,...
                          (-dvy+constants.wE*dx)/rho+(dy*N)/rho^3,...
                           -dvz/rho+(dz*N)/rho^3];
        % Rotate to ecef
        prhopRs= prhopRs_eci*R3;
        prhopdotRs = prhopdotRs_eci*R3;
        break
    end
end

Htilde = [dx/rho, dy/rho, dz/rho, 0, 0, 0, 0, ... % Row 1, Columns 1-12
          prhopRs, 0, 0, 0; % Row 1, Columns 10-end
          dvx/rho-(dx*N)/rho^3, dvy/rho-(dy*N)/rho^3, dvz/rho-(dz*N)/rho^3,... % Row 2, Columns 1-3
          dx/rho, dy/rho, dz/rho, 0,... % Row 2, Columns 4-12
          prhopdotRs, 0, 0, 0]; % Row 2, Columns 10-end
