function [Xhat,Xhatm,dxhat,dxhatm,P,Pm,y,yhat,yhatl] = SRIF2(t,xbar0,Pbar0,Q,Y,V,X0,constants,stations,num_iterations)
%{
Inputs:
    >t: time vector
...
for j = 1:num_iterations
    ...
    [~,Xint] = ode45(@(t,x) odeSTM_SunEarth3BP_SRP2(t,x,constants),t,[X0';reshape(eye(n),[],1)],options);
    Xbar = Xint(:,1:n);
    ...
    for i = 1:length(t)
        ...
        current_station = [current_id, Xbari(9+3*(idx-1)+(1:3))];
        ...
        y(:,i,j) = M(:,3:4)' - G(:,2:3)';
        Htilde(:,:,i,j) = linearizedH2(t(i),Xbari,[current_id;gs_state],constants,station_ids);
