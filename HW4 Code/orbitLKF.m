function [Xhat,dxhat,P,Pbari,y,yhat,Phii] = orbitLKF(t,xbar0,Pbar0,Q,Qframe,Y,R,Xnom,constants,stations,num_iterations)
%{
Inputs:
    >t: time vector
...
for j = 1:num_iterations
    ...
    for i = 1:length(t)
        ...
        Ki = Pbari(:,:,i,j)*Htilde'/(Htilde*Pbari(:,:,i,j)*Htilde' + R);
    
        % Measurement correction
        dxhat(:,i,j) = xbari + Ki*(y(:,i,j)- Htilde*xbari);
        P(:,:,i,j) = (eye(n) - Ki*Htilde)*Pbari(:,:,i,j)*(eye(n) - Ki*Htilde)' + Ki*R*Ki';
