function [Xhat,P,y,yhat,batch_cnt] = orbitBatch(t,xbar0,Pbar0,Y,R,Xnom,constants,stations,tol,num_iterations,data_type)
%{
Inputs:
    >t: time vector
    ...
    >num_iterations: number of times to iterate 
...
% Iterative algorithm
while err > tol || batch_cnt < num_iterations
    k = batch_cnt + 1;
    ...
    [~,Xint] = ode45(@(t,x) odeSTM_J2(t,x,constants),t,[Xbari';reshape(Phii,[],1)],options);
    Xbar = Xint(:,1:n);
    ...
    for i = 1:length(t)
        ...
        [G,gs_state(:,i)] = genSingleMeasurement(t(i),current_station,constants,Xbari(i,1:6));

        % Prefits and update
        Htilde = linearizedH(Xbari(i,1:6),gs_state(:,i));
        if data_type == "range"
            R_1 = R(1,1);
            Htilde = Htilde(1,:);
            y(:,i,k) = M(3)' - G(2)';
